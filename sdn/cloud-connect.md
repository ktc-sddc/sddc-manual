...
# Shared Colocation Gateway 관리(이미지 변경)
...
1. 메뉴 SDN 관리 > Shared Colocation Gateway 관리 **\[신청 목록 관리]** 탭에서, 신청 내역서의 **\[승인]** 버튼을 클랙합니다.
...
1. 메뉴 SDN 관리 > Shared Colocation Gateway 관리 **\[신청 목록 관리]** 탭에서, 신청 내역서의  **\[반려]** 버튼을 클릭합니다.
...
3. 처리 완료 내역에서 반려한 신청이 조회되는지 확인합니다.
...
2.  Shared Colocation Gateway  생성 정보를 입력합니다.
    ...
    3. System Tenant: Cloud Connect 가 생성될 System Tenant 입니다.
...
5. 메뉴 SDN 관리 > Shared Colocation Gateway 관리 **\[신청 목록 관리]** / **\[전체]** 탭에서 해당 신청 내역의 처리 상태가 "회수" 상태로 변경 되었는지 확인합니다.(신청을 통해 사용 권한을 획득한 경우)
